...
**These are roles, not security boundaries, in v1.** The v1 realization runs all three in one process, against one working copy, with one shell. A statement that the verifier "has no access to modify the target codebase" would be false in that topology — it would be a prompt convention described as a capability constraint. `research/G-trust-layer-reconciliation.md` already says this in plain language: *"there is a seam by construction."* EXEC-2 states what is actually true and enforceable in v1; EXEC-10 (v2) names the mechanism that would make it a real boundary.
...
- **[v1]** — in scope for the first working system. 28 IDs.
...
**v1 index** (28): AUDIT-1 · PLAN-1 · PLAN-5 · EXEC-1 · EXEC-2 · EXEC-3 · EXEC-6 · EXEC-7 · QA-1 · QA-2 · QA-5 · APPROVAL-1 · APPROVAL-5 · INTEGRITY-1 · INTEGRITY-2 · INTEGRITY-3 · INTEGRITY-8 · INTEGRITY-9 · ESCALATE-1 · ESCALATE-2 · DIAG-1 · SECRET-1 · DISCLOSE-1 · ENV-1 · BUDGET-1 · BUDGET-2 · TEST-1 · PROCESS-1

*(PROCESS-1 governs the control plane's own codebase rather than a run, and so is excluded from the run-level acceptance criteria in §6; the runtime v1 set is the other 27. The review's estimate was ~15 v1 requirements. This lands at 28 because 28 IDs now carry what 63 carried before — AUDIT-1 alone absorbs three former requirements, PLAN-1 absorbs three, EXEC-6 absorbs four. The cost-weighted v1 surface is smaller than the count suggests; padding was resisted, merging was not.)*
...
- **EXEC-10** **[v2]** — The verifier SHALL execute against an independent checkout of the run branch that the implementer cannot write to (separate working directory at minimum; read-only bind mount into a separate container preferably), making EXEC-2 a capability boundary rather than a configuration constraint. *This is the honest mechanism for what former QA-4 merely asserted. It is v2 only because v1's adversary is a confused model, not a malicious one (§5), and INTEGRITY-3 already detects the failure. Trigger to promote: the first real scope violation that INTEGRITY-3 catches originating from the verifier role, or any multi-tenant / untrusted-repo use.*
...
- **QA-4** — *merged into EXEC-2 (honest restatement) and EXEC-10 (the v2 mechanism).*
...
- **ENV-3** **[v2]** — Implementer and verifier code execution SHOULD occur in an ephemeral, isolated environment treated as running untrusted code, destroyed at run end or escalation.
  **⚠ Unresolved contradiction with the reference implementation — read before promoting.** The reference migration's *source* state is .NET Framework, and Linux containers cannot run legacy .NET Framework MSBuild (`IMPLEMENTATION-PLAN.md` Phase 7 already records this). So the single most important verification in the system — *does the pre-change code still build?* (QA-5's baseline capture) — cannot be performed inside the sandbox ENV-3 would mandate, and would fall through QA-1's `not-run` path on every run. Windows containers are technically possible on the target Windows 11 Pro host but are multi-GB, slow, and require Hyper-V isolation. **This is a one-afternoon spike that must happen before ENV-3 is ever written as a SHALL.** For v1 the isolation story is INTEGRITY-9's dedicated run branch plus INTEGRITY-3's scope check, against a repository the operator already trusts, on the operator's own box — roughly 90% of the safety for 2% of the cost. Trigger to promote: running against a repository the operator does not fully trust, or any multi-tenant use.

- **ENV-4** **[v2]** — Network egress from the execution environment SHOULD default to denied with an explicit allowlist.
  **⚠ Two known blockers, recorded so promotion does not happen blind.** (1) `nuget.org` is allowlistable; private Azure Artifacts / Artifactory feeds are not, without credentials — and those credentials live in `NuGet.config`, precisely the kind of file the deleted INTEGRITY-5 forbade reading. Nothing in v1 covers authenticated private package feeds, and that is a hard blocker for any real .NET enterprise codebase (§10.6). (2) ENV-4 presupposes ENV-3's environment boundary, which is itself blocked. Trigger to promote: ENV-3 promoted, and an authenticated-private-feed story that works.
...
**Not in the model at all for v1**: multi-tenant use, untrusted target repositories, network attackers, and supply-chain attacks on the control plane's own dependencies. Each of those, if it becomes real, promotes a specific deferred requirement (ENV-3, ENV-4, KNOWLEDGE-4's rejected half) rather than requiring a redesign.
...
1. **Does the .NET Framework toolchain function under any container isolation at all?** (ENV-3). Answer determines whether ENV-3 is ever promotable or should become a permanent won't-do for this domain. Run the baseline build of a real .NET Framework solution in a Windows container on the target host and time it.
2. **Can the verifier be given an independent, non-writable view of the run branch cheaply?** (EXEC-10). A second `git worktree` checked out to the run branch, plus a verifier process whose tools cannot write, is probably 30 lines. Confirm or refute.
