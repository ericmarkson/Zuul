...
**Accepted redesign, not yet built as of this note** (full detail in `CLAUDE.md`'s "ARCHITECTURAL PIVOT IN PROGRESS" section and `logs/session-log.md`):
- **`PLAN-1` path (a)'s mechanism changes from template-matching to one model call per finding-group**, proposing `side_effect_class`, description, refined `declared_scope`, and check(s) for that specific remediation — still producing the identical on-disk plan shape `PLAN-1` already requires, still frozen at the single `APPROVAL-1` gate before any execution, still executed by an unmodified engine. Grouping itself (`PLAN-2`'s v1 lexical scope, category + path-overlap) is unaffected.
- **The per-phase implementer (`EXEC-2`'s "implementer" role) becomes a bounded multi-step tool-using agent** rather than one completion call — can read additional files for context before finalizing edits, still hard-bounded by `BUDGET-1/2` and `EXEC-3`'s retry count, still only able to write within that phase's already-frozen scope.
- **`QA-2` is explicitly reaffirmed, not reopened.** The redesign moves who authors a check (a model, at generation time, reviewed at the gate) — never how a check is evaluated (still a real exit code / machine-readable artifact, never a second model's opinion of the first model's work). An LLM "QA agent" whose verdict is the actual pass/fail gate would violate `QA-2`; this was flagged explicitly during the design discussion precisely because "QA" is easy to say in a way that implies exactly that.

Not superseded: `INTEGRITY-1/2/3/9`, `APPROVAL-1/5`, `EXEC-6/7`, `BUDGET-1/2`, `SECRET-1`, `DISCLOSE-1`, `PLAN-5`'s per-phase-checks capability (newly built, `b25eda5` — this is exactly the mechanism the redesign needs to attach a model-authored check to a model-authored phase). Superseded: the specific claim, nowhere formally stated as a requirement but implicit in how Phase D/C.md's earlier notes described the pack, that a remediation must correspond to a pre-authored template file to become a phase at all.
