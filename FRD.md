...
- **EXEC-10** **[v2]** — The verifier SHALL execute against an independent checkout of the run branch that the implementer cannot write to (separate working directory at minimum; read-only bind mount into a separate container preferably), making EXEC-2 a capability boundary rather than a configuration constraint. *This is the honest mechanism for what former QA-4 merely asserted. It is v2 only because v1's adversary is a confused model, not a malicious one (§5), and INTEGRITY-3 already detects the failure. Trigger to promote: the first real scope violation that INTEGRITY-3 catches originating from the verifier role, or any multi-tenant / untrusted-repo use.*
  *Elevated by ENV-3's 2026-09-18 resolution: with verify staying host-executed and unsandboxed by design, a second `git worktree` (§8 item 2 — probably 30 lines) is now the cheapest real separation mechanism available, independent of whether ENV-5's implementer container ever gets built. Worth building before ENV-5, not after.*
