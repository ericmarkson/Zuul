...
- **KNOWLEDGE-1** — *split, 2026-09-21.* The original claim ("domain-specific knowledge SHALL be served from a separate component via MCP") stays **[v2]** in its literal form: the pack itself (the analyzer, templates) is still a local versioned directory + CLI, not an MCP server, per the v1 decision below. The originally-written promotion trigger — *the implementer model needing to call the pack's own tools live, mid-phase, with arguments not known at plan time* — has still not occurred and remains v2/unmet.

  **A related but distinct case was promoted to [v1] instead**, live-built the same day this note was written: **plan generation** (`plangen_llm.py`'s researcher, not the implementer) may consult one or more **operator/pack-declared remote MCP servers** — third-party authoritative sources (e.g. a documentation search), not the knowledge pack's own tools — before `PLAN-5` freezes a phase's scope and checks. This is a stronger case for MCP than the original trigger anticipated: it is external knowledge no local package could sensibly vendor (a live, current documentation index), consulted at exactly the point in the pipeline (before the freeze) where research is still actionable, generic across any domain (the CLI takes a URL/label, never a hardcoded provider), and additive — no MCP server configured means zero behavioral change from before. `controlplane/model_provider.py` gained `complete_with_mcp` (the Responses API's native remote-MCP tool support; OpenAI's own infrastructure executes the remote call, this project's code never sees or dispatches it) and `plangen_llm.py` gained a parallel research-loop path using it. Original decision on the pack itself, still true: a local, versioned directory (a CLI that emits the AUDIT-1 artifact, plus template files), identified by content hash in the event log (EXEC-6). Trigger to promote *that* case: still the original one, unmet.
...
## 14. Revision note — 2026-09-20 architectural pivot (plan generation and implementation, in progress)

Phase F's first real run (against `alloy-mvc-template`, live model) found two gaps: `incompatible-api` findings scoped only to where the analyzer's evidence lived, not real usage sites; and no mechanism to catch a phase that deleted content without producing what it promised, since `QA-5`'s delta check only asks whether the build got worse. Both were fixed directly — real usage-site detection, and a template-declared `expected_new_paths` field synthesizing a generated existence check per phase (commit `b25eda5`).

The user rejected the *shape* of the second fix before it was re-verified live: a fixed node-template catalogue that a finding's `remediation_tag` must match, with per-template fields hardcoded in advance, is the wrong kind of system for what this project is meant to be. Clarified boundary: assessment-artifact production (`AUDIT-4`'s "one producer," the knowledge pack) is entirely external to the control plane — this project's actual scope starts at the audit and covers turning findings into phases, executing each phase via a runtime agent, and verifying each was done correctly.

**Redesign built, 2026-09-20, same day** (full detail in `CLAUDE.md`'s decision log and `IMPLEMENTATION-PLAN.md`'s Phase D/E sections):
- **`PLAN-1` path (a)'s mechanism changed from template-matching to one model call per finding-group** (`controlplane/plangen_llm.py`), proposing `side_effect_class`, description, refined `declared_scope`, and check(s) for that specific remediation — still producing the identical on-disk plan shape `PLAN-1` already requires, still frozen at the single `APPROVAL-1` gate before any execution, still executed by an unmodified engine. Grouping itself (`PLAN-2`'s v1 lexical scope, category + path-overlap) is unaffected. The fixed node-template catalogue and `plangen.py`'s `load_templates` path are deleted, not extended; the four templates were deliberately not kept as few-shot context, since they encoded exactly the failure modes Phase F's first run found. **Live-validated against the real `alloy-mvc-template` audit**: the model-proposed phase-3 `declared_scope` correctly included the actual `.cs` usage-site files (not just the `.csproj`'s `<Reference>` list), and phase-4 correctly proposed `appsettings.json` in `additional_scope` plus a generated check verifying it's valid JSON and the legacy config files are gone — direct evidence against both of Phase F's original gaps, produced dynamically rather than hardcoded.
- **The per-phase implementer (`EXEC-2`'s "implementer" role) became a bounded multi-step tool-using agent** (`controlplane/llm_implementer.py`'s `request_edits`) rather than one completion call — a new `ModelProvider.complete_with_tools` gives it `read_file`/`list_directory` (path-traversal-guarded to the target repo) before it must call `finalize_edits` exactly once, still hard-bounded by `BUDGET-1/2`, `EXEC-3`'s retry count, and a new `max_tool_rounds` ceiling, still only able to write within that phase's already-frozen scope via the same single write path `runner.py` already scope-checks. **Live-verified against the real API**: found and fixed a real bug the first time this touched `gpt-5.6-sol` for real — its chat-completions endpoint rejects function tools combined with any non-`"none"` `reasoning_effort`.
- **`QA-2` was reaffirmed, not reopened**, exactly as planned. The redesign moved who authors a check (a model, at generation time, reviewed at the gate) — never how a check is evaluated (still a real exit code / machine-readable artifact, never a second model's opinion of the first model's work).

Not superseded: `INTEGRITY-1/2/3/9`, `APPROVAL-1/5`, `EXEC-6/7`, `BUDGET-1/2`, `SECRET-1`, `DISCLOSE-1`, `PLAN-5`'s per-phase-checks capability (`b25eda5` — exactly the mechanism the redesign needed to attach a model-authored check to a model-authored phase). Superseded: the specific claim, nowhere formally stated as a requirement but implicit in how Phase D/C's earlier notes described the pack, that a remediation must correspond to a pre-authored template file to become a phase at all.
